...
public class Bot {
     ...
     **/
    public String run() {
        /* Greedy Strategy by LebahGanteng Test Bot 5 */
        String command = "";
        ...
        if (!energyIsEnough()) {
            ...
            for (int i = 0; i < gameState.getGameDetails().mapHeight; i++) {
                ...
                if (enemyAttackOnRow + enemyDefenseOnRow <= 0) {
                    command = placeBuildingInRow(BuildingType.ATTACK, i);
                }
            }
        }
     ...
     * @return the result
    */
    private boolean energyIsEnough() {
        int myEnergy = getEnergy(PlayerType.A);
        int energyBuildings = 0;

        for (int i = 0; i < gameState.getGameDetails().mapHeight; i++) {
            energyBuildings += getAllBuildingsForPlayer(PlayerType.A, b -> b.buildingType == BuildingType.ENERGY, i).size();
        }

        return (myEnergy >= 30 || energyBuildings > 8);
    }
     ...
     * @return the result
     **/
    private Boolean needBuildTesla() {
        int myTeslaBuildings = 0;

        for (int i = 0; i < gameState.getGameDetails().mapHeight; i++) {
            myTeslaBuildings += getAllBuildingsForPlayer(PlayerType.A, b->b.buildingType == BuildingType.TESLA, i).size();
        }

        return myTeslaBuildings < 3;
    }
