...
public class Bot {
    ...
    public String run() {
        ...
        } else if (canAffordBuilding((BuildingType.TESLA)) && needBuildTesla()) {
            command = placeBuildingRandomlyFromBack((BuildingType.TESLA));
        }
    ...
    private Boolean needBuildTesla() {
        ...
        for (int i = 0; i < gameState.getGameDetails().mapHeight; i++) {
            myTeslaBuildings += getAllBuildingsForPlayer(PlayerType.A, b->b.buildingType == BuildingType.ENERGY, i).size();
        }
